...
get_all_keys(GConfClient *client, std::vector<GObjectWrapper<GConfEntry> >& keys, std::string dir,
  ...
  if (children) {
	...
	// filter against locked_keys
	std::string needle = gconf_entry_get_key(key);

	if (std::find(locked_keys.begin(), locked_keys.end(), needle) == locked_keys.end()) {
	  keys.push_back(GObjectWrapper<GConfEntry>(key));
	} else {
	  std::cout << "Skipping locked key " << needle << std::endl;
	}
      }
...
static void
get_locked_keys(const std::string& path, std::vector<std::string>& keys) {
  std::cout << "Reading locked keys from " << path << std::endl;
  ...
  while (std::getline(file, key)) {
    // This is very crude but it's what dconf update does :p
    if (key.size() > 1 && key[0] == '/') {
      keys.push_back(key);
      std::cout << "  Found locked key " << key << std::endl;
      key.clear();
    }
  }
...
get_locked_keys () {
  // This is really crude but we are basically going to mimic what dconf update does when
  // it gets the locked keys

  std::vector<std::string> dirs;
  dirs.push_back("/etc/dconf/db/nemo.d/locks/");
  dirs.push_back("/etc/dconf/db/vendor.d/locks/");

  std::vector<std::string> locked_keys;

  for (int x = 0; x < dirs.size(); x++) {
    DIR *d = opendir(dirs[x].c_str());
    if (!d) {
      continue;
    }
...
main(int argc, char *argv[]) {
  ...
  std::vector<std::string> locked_keys;
  try {
    locked_keys = get_locked_keys();
  } catch (...) {
    std::cerr << "Failed to read locked keys" << std::endl;
  }

  std::vector<GObjectWrapper<GConfEntry> > keys;
  bool have_all_keys = get_all_keys(*gconf, keys, "/", locked_keys);
